...
alter table public.weekly_updates
  add constraint weekly_updates_content_format_check
  check (content_format in ('video', 'chart', 'text'));

alter table public.weekly_updates
  add constraint weekly_updates_body_length_check
  check (body is null or char_length(body) <= 12000);

alter table public.weekly_updates
  add constraint weekly_updates_image_paths_check
  check (
...
alter table public.weekly_updates
  add constraint weekly_updates_published_non_video_check
  check (
    is_published is not true
    or content_format = 'video'
    or (
      type = 'market_update'
